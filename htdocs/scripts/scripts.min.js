/* chargement au début de l'application, dans index.html **/
function Reset() {
  localStorage.clear();
}
...
$('.btn_print, .print').click(function(){
      //print();
      $.ajax({
        method: "POST",
        url: "../imposition/insert.php",
        data: {
          "pseudo" : localStorage.pseudo,
          "email" : localStorage.email,
          "svg" : myBadge,
          "png" : localStorage.fileName
        }
      }).done(function(datas) {
        //alert(datas)
      });
      localStorage.print = "y";
});
...
$('.print').on('click', function(){
    ...
    if (localStorage.mail != 'y') {
    $('.li_pop_2').css("display", "block");
  }
     if (localStorage.facebook != 'y') {
...
$('#shareBtn, .face').on('click', function(e){
    var elementLS = window.localStorage.getItem("filename");
    e.preventDefault();
    alert('facebook');
    localStorage.facebook = "y";
    FB.init();
  FB.ui({
    method: 'share',
    //display: 'iframe',
    mobile_iframe: true,
    //app_id: 184484190795,
    href: 'https://killer-cepegra.xyz/badges/htdocs/badges/'+elementLS+'.png',
  }, function(response){
      //return false;
    $('.li_pop_1').css("display", "block");
    $('.li_pop_2').css("display", "block");
...
/* SEND EMAIL */
$(".btn_mail, .mail").on('click', function(e){
  e.preventDefault();
